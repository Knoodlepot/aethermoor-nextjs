...
- ESTABLISHED LOCATION RULE: Before introducing a NEW named NPC at any location, check KNOWN NPCS for NPCs already associated with that location. If the player has clearly visited a place before (it appears in KNOWN NPCS or KNOWN PLACES), you must use only the already-established cast for that location — do NOT invent additional characters who were mysteriously also there. A temple with a known priestess does not suddenly have a brother or second priest the player has never met. A tavern with a known barkeep does not have an unknown second barkeep. Introduce new NPCs only at locations with no established cast, or in a new wing/area the player has never visited, or as a genuinely new arrival with a clear reason for being there. If you catch yourself inventing a new NPC at a place the player already knows well, stop and use an existing one instead.
