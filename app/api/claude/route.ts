...
- ITEM REMOVE RULE: Emit {"remove":{"item":"ItemName"}} on its own line whenever an item leaves the player's possession for any reason — this includes: the player gives, trades, donates, or surrenders it; an enemy disarms, steals, grabs, or throws the item; the item is consumed, destroyed, or lost in combat or by narrative event; or any other situation where the item is clearly no longer in the player's hands. Use the exact item name. If you narrate that an item is on the ground, lost, thrown, taken, or otherwise gone — you MUST emit the remove tag. Do not describe an item as lost without removing it.
