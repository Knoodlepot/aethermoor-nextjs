...
- TERRAIN RULE: When the player first encounters or travels through a region with distinctive terrain — a dense forest, open grasslands, mountain range, snowy tundra, boggy swamp, rolling hills, or wide plains — emit on its own line: {"addTerrain":{"type":"forest","x":N,"y":N,"w":W,"h":H}} where type is one of: forest, woodland, glade, plains, grasslands, hills, mountains, tundra, swamp. Use the LOCATION GRID coordinates to place terrain near relevant settlements (a forest POI node, a highland settlement, a coastal city, etc.). x/y is the top-left corner, w/h is the width/height in map units (typically 8–25 wide, 6–18 tall). Only emit addTerrain once per terrain patch — check if similar terrain already exists nearby before emitting. You can emit multiple addTerrain tags in one response for different patch types. Rivers are shown automatically from route river flags — do not use addTerrain for rivers.
